...
import Link from 'next/link'; // Assuming Next.js, otherwise use a standard <a> with relative path
...
const ProfileSidePanel = ({ isOpen, onClose, data, loading }: ProfileSidePanelProps) => {
  ...
  // Reusable Component for Data Rows with Copy functionality
  const InfoRow = ({ label, value, copyValue }: { label: string, value: any, copyValue?: any }) => (
      ...
      <div className="fixed inset-y-0 right-0 w-full md:w-[480px] bg-white shadow-2xl z-[70] flex flex-col animate-in slide-in-from-right duration-300">
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                  ...
                  <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    <div className="px-4 py-3 bg-gray-50 border-b border-gray-100 flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-black text-gray-600 uppercase">Account Lifecycle</span>
                    </div>
                    <div className="p-4">
                      <InfoRow label="Current Onboarding Step" value={data?.step?.replace(/_/g, ' ')} copyValue={data?.step} />
                      <InfoRow label="KYC Status" value={data?.kycStatus} />
                      <InfoRow label="Member Since" value={data?.createdAt ? new Date(data.createdAt).toLocaleString() : null} copyValue={data?.createdAt} />
                    </div>
                  </div>
                  ...
                  {data?.cards && data.cards.length > 0 && (
                    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                      <div className="px-4 py-3 bg-gray-50 border-b border-gray-100 flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-black text-gray-600 uppercase">Linked Payment Methods</span>
                      </div>
                      <div className="p-4">
                        {data.cards.map((card: string, idx: number) => (
                          <InfoRow key={idx} label={`Card ${idx + 1}`} value={card} />
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {activeTab === 'Risk' && (
                <div className="space-y-6 animate-in fade-in">
                  <div className="bg-white p-6 rounded-xl border shadow-sm text-center">
                    ...
                    <div className={`mt-4 inline-block px-4 py-1 rounded-full text-xs font-black uppercase ${data?.riskScore?.riskLevel === 'Low' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {data?.riskScore?.riskLevel ?? 'null'} Level
                    </div>
                  </div>
                  <div className="bg-white p-4 rounded-xl border shadow-sm space-y-2">
                    <InfoRow label="Country Score" value={data?.riskScore?.scores?.countryScore + '%'} />
                    <InfoRow label="Transaction Volume Score" value={data?.riskScore?.scores?.transactionsScore + '%'} />
                    <InfoRow label="Account Status Score" value={data?.riskScore?.scores?.accountStatusScore + '%'} />
                  </div>
                </div>
              )}
              ...
              {activeTab === 'Transactions' && (
                <div className="space-y-6 animate-in fade-in">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-white p-5 rounded-xl border shadow-sm">
                      <p className="text-[10px] font-black text-gray-400 uppercase">Last Activity</p>
                      <p className="text-sm font-bold text-gray-800 mt-1">{data?.transaction?.lastTransactionDate ? new Date(data.transaction.lastTransactionDate).toLocaleDateString() : 'Never'}</p>
                    </div>
                  </div>
       ...
       <div className="bg-white p-6 rounded-2xl border shadow-sm grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-green-100 to-green-50 p-5">
                ...
                {data?.transaction?.totalTransactions?.successfulTransactions}
                </div>

                {/* subtle icon */}
                <div className="absolute right-4 top-4 text-green-300">
                ✓
                </div>
            </div>
            ...
            <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-red-100 to-red-50 p-5">
                ...
                {data?.transaction?.totalTransactions?.failedTransactions}
                </div>
                ...
                ✕
                  ...
                  <div className="pt-2">
                    ...
                    <p className="text-[10px] text-center text-gray-400 mt-3 font-medium uppercase tracking-tighter">
                      Navigation to transactions for User Id #{data?.userId}
                    </p>
                  </div>
                </div>
              )}
                  ...
                  {data?.documents?.map((doc: any, idx: number) => (
                    <div key={idx} className="bg-white p-5 rounded-xl border shadow-sm">
                      <div className="flex items-center gap-3 mb-4 border-b pb-3">
                        ...
                        <div>
                          <p className="text-sm font-black uppercase text-gray-900">{doc?.type ?? 'null'}</p>
                          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">{doc?.issuingCountry ?? 'null'}</p>
                        </div>
                      </div>
