...
const formatMessageTimestamp = (timestampStr) => {
...
export default function Conversation({ selectedChat, setSelectedChat }) {
  ...
  // CHANGED: This hook now resets state when the chat changes.
  useEffect(() => {
    if (selectedChat?.id) {
    ...
    // Reset conversation-specific state when switching chats
    setNewMessage('');
    setShowEmojiPicker(false);
    setActiveNoteEditorId(null);
    // You might want to reset notes if they are not persisted on a server
    // setMessageNotes({});
        ...
        <>
                  ...
                  return (
                    <Fragment key={msg.id}>
                      ...
                      <div className={`flex flex-col ${isSent ? 'items-end' : 'items-start'}`}>
                          ...
                          {activeNoteEditorId === msg.id && (
                            <div className="w-full max-w-xl mt-2 p-2 border border-blue-300 bg-blue-50/50 rounded-lg">
                                <textarea ref={noteTextareaRef} value={currentNoteText} onChange={(e) => setCurrentNoteText(e.target.value)} rows={2} className="w-full bg-transparent text-sm text-gray-800 focus:outline-none resize-none" placeholder="Add a note..."/>
                                <div className="flex justify-end gap-2 mt-2">
                                    <button onClick={handleCancelNote} className="px-3 py-1 text-sm text-gray-600 hover:bg-gray-200 rounded-md">Cancel</button>
                                    <button onClick={handleSaveNote} className="px-3 py-1 text-sm text-white bg-blue-600 hover:bg-blue-700 rounded-md">Save</button>
                                </div>
                            </div>
                          )}
                          {messageNotes[msg.id] && activeNoteEditorId !== msg.id && (
                            <div className="w-full max-w-xl mt-2 p-2 bg-yellow-100/60 border-l-4 border-yellow-400 rounded-r-lg">
                                <div className="flex items-start gap-2">
                                    <NotebookPen className="w-4 h-4 text-yellow-600 mt-0.5 flex-shrink-0" />
                                    <p className="text-sm text-gray-700 italic">{messageNotes[msg.id]}</p>
                                </div>
                            </div>
                          )}
                      </div>
          ...
          {selectedChat.isClosed ? (
            <div className="pt-4 text-center">
              <p className="text-sm text-gray-500 bg-gray-100 p-3 rounded-lg">This conversation is closed.</p>
            </div>
          ) : (
            <div className="pt-4">
