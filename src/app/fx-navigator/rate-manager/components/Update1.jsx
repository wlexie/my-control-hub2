...
import UpdateWeighted from "./UpdateWeighted"; // Success modal
...
const Update1 = ({
  ...
  // API call without utils
  const handleSetNewRate = async () => {
    setIsLoading(true);
    try {
      const response = await axios.put(
        "http://tuma-dev-backend-alb-1553448571.us-east-1.elb.amazonaws.com/api/treasury/new-interbank-rate",
        null,
        {
          params: {
        ...
        >
          ...
          <div className="flex flex-col h-full p-6 overflow-y-auto">
            ...
            <div className="flex justify-between items-start mb-6">
              <div className="flex-1">
                ...
                <p className="text-gray-500 text-base font-medium">
                  Current Rate: 1 {baseCurrency?.code} = {rateValue}{" "}
                  {targetCurrency?.code}
                </p>
              </div>
            ...
            <div className="bg-white rounded-xl items-center flex flex-wrap justify-center gap-2 p-4">
              <p className="text-base font-bold mr-4 w-full md:w-auto text-center md:text-left">
                Current Bank Rate
              </p>
              ...
              <span className="border items-center flex rounded-lg p-1 md:p-2">
                ...
                <span className="px-2 rounded-lg flex items-center md:gap-1 md:gap-2 py-1 bg-[#F3F5F8]">
                  <Image
                    ...
                    alt={baseCurrency?.code || "flag"}
                    width={24}
                    height={16}
                    className="rounded-sm w-[18px] md:w-[24px] mr-1"
                  />
              ...
              <span className="border items-center flex rounded-lg p-1 md:p-2">
                <input
                  ...
                  className="font-semibold text-lg md:w-24 w-15 pl-2 outline-none"
                />
                <span className="bg-[#F3F5F8] rounded-lg flex items-center mx-2 px-2 py-1 md:gap-2">
                  <Image
                    src={getFlagUrl(targetCurrency?.country)}
                    alt={targetCurrency?.code || "flag"}
                    width={24}
                    height={16}
                    className="rounded-sm md:w-[24px] mr-1 w-[18px]"
                  />
            ...
            <div className="mt-auto flex flex-col md:flex-row gap-4 pt-6">
              {isLoading ? (
                ...
                <>
                  <button
                    onClick={onClose}
                    className="w-full md:w-1/2 py-3 text-base font-semibold text-[#276EF1] border-[#276EF1] border rounded-lg hover:bg-blue-50 transition"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={handleSetNewRate}
                    className="w-full md:w-1/2 py-3 text-base font-semibold text-white bg-[#276EF1] rounded-lg hover:bg-blue-700 transition"
                  >
                    Set New Rate
                  </button>
                </>
