...
const ModuleCard: React.FC<ModuleCardProps> = ({ name, iconSrc, title, description, onSelect }) => {
  return (
    <div className="bg-white rounded-xl border p-6 flex flex-col items-center  h-full transition-all duration-300 hover:shadow-lg">
      <div className="mb-4 w-12 bg-indigo-50 p-3 rounded-full h-12 relative"> {/* Adjust size as needed */}
        <Image
          src={iconSrc}
          alt={`${name} icon`}
      ...
      <span className="text-[14px] text-center font-[600] text-[#2A282F]">{title}</span>
      ...
      <button
        onClick={onSelect}
        className="mt-auto w-full bg-indigo-50 hover:bg-indigo-100 text-indigo-600 font-medium py-2 px-4  
